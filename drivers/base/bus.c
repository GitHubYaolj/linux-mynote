...
int bus_add_device(struct device *dev)
{
	...
	if (bus) {
		...
		error = device_add_attrs(bus, dev);//将由bus->dev_attrs指针定义的默认attribute添加到内核中，它们会体现在/sys/devices/xxx/xxx_device/目录中
		if (error)
			goto out_put;
		error = sysfs_create_link(&bus->p->devices_kset->kobj,
						&dev->kobj, dev_name(dev));//将该device在sysfs中的目录，链接到该bus的devices目录下
		if (error)
			goto out_id;
		error = sysfs_create_link(&dev->kobj,
				&dev->bus->p->subsys.kobj, "subsystem");//在该设备的sysfs目录中中，创建一个指向该设备所在bus目录的链接
		if (error)
			goto out_subsys;
...
//    那么就会调用driver_attach(drv)
	...
	error = driver_add_attrs(bus, drv);//在sysfs的该driver的目录下，创建由bus->drv_attrs指针定义的默认attribute
	if (error) {
...
int bus_register(struct bus_type *bus)
{
	...
	retval = kset_register(&priv->subsys);//该接口同时会向sysfs中添加对应的目录（如/sys/bus/spi）
	if (retval)
		goto out;
	...
	retval = add_probe_files(bus);//在目录/sys/bus/spi/drivers下添加drivers_autoprobe和drivers_probe
	if (retval)
		goto bus_probe_files_fail;

	retval = bus_add_attrs(bus);//添加由bus->bus_attrs指针定义的bus的默认attribute，这些attributes最终会体现在/sys/bus/xxx目录下
	if (retval)
		goto bus_attrs_fail;
